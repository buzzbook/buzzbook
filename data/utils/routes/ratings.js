...
Router.get("/byCourse", (req, res) => {
  ...
  const query = 'SELECT AVG(`Course: Overall Effectiveness`) AS courseEff, AVG(`Hours per Week`) as hoursPer FROM byCourse WHERE Dept = ? AND Num LIKE ? AND Year >= 2017; SELECT AVG(`Instructor: Overall Effectiveness`) AS profEff FROM byProf WHERE Dept = ? AND Num LIKE ? AND Year >= 2017'
